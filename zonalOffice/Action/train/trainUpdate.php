...
<?php
    if(isset($_POST['submit'])){
        ...
        $Ttstsvar = base64_encode($Ttsts);

        if($_SESSION['ref']==1){
            $update="UPDATE tstbl SET tsts='$Ttstsvar' where tid=$id ";
        }elseif($_SESSION['ref']==5){
            $update="INSERT INTO tstbltemp(tid,tsts)VALUES('$id','$Ttstsvar') ";
        }
...
<Script>
    ...
    alert("Training Status Details Updated!");
        ...
        <div style="background-color:#acb0bd;" class="page-wrapper">
            <div class="container-fluid">
                ...
                <div class="col-lg-2 col-sm-4 col-xs-12">
                    <a href = "../../htmlPages/clientSide/trastsView.php"><button class="btn btn-block btn-primary">Click to Back</button></a>
                </div>
                <br><br>
                <h1 style="color:#000000;text-align:center;"><strong><em>Update Training status</em></strong></h1><br>
                <div class="row">
                <div class="col-md-6">
                        <div class="white-box">
                                    ...
                                    <form action="" method="post">
                                        <?php
                                            while($recordtq = mysqli_fetch_array($resulttq)){
                                                ...
                                                $debood = base64_decode($bood);

                                                $sqlnd="SELECT * FROM teacher where tid = $bbtid ";
                                                $resultnd=$conn->query($sqlnd);
                                                while($recordnd = mysqli_fetch_array($resultnd)){
                                                    $bbnic = $recordnd['nic'];
                                                    $debbnic = base64_decode($bbnic);
                                                }
                                        ?>
                                        ...
                                        <label for="">NIC</label>
                                            <div class="input-group">
                                                ...
                                                <label for="" class="form-control"><?php echo $debbnic;?></label>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="Ocaid">Training Status</label>
                                            <div class="input-group">
                                                <div class="input-group-addon"><i class="fa fa-square-o" aria-hidden="true"></i></div>
                                                <select class="form-control" id = "Otsts1" name="Otsts" required>
                                                    <option value="Trained" <?php if($debood=="Trained"){echo 'selected';}?>>Trained</option>
                                                    <option value="Not Trained" <?php if($debood=="Not Trained"){echo 'selected';}?>>Not Trained</option>
                                                </select>
                                            </div>
                                        </div>
                                        <?php
