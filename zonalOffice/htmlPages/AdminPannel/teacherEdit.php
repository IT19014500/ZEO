...
<?php
  $sqljub="SELECT * FROM nonprincipletemp";
  $resultjub=$conn->query($sqljub);
?>

<?php
  $sqldef="SELECT * FROM nonprincipletmdel";
  $resultdef=$conn->query($sqldef);
?> 

<?php
if(isset($_GET['delnprid'])){

    $denpo = $_GET['delnprid'];

    $deletetnact="delete from nonprinciple where nprid= $denpo";
    $conn->query($deletetnact);

    $deletetnvb="delete from nonprincipletmdel where nprid= $denpo";
...
<?php
    if(isset($_GET['nprid'])){
        ...
        $sqledte="SELECT * FROM nonprincipletemp WHERE nprid = $bytedi";
        $resultedte=$conn->query($sqledte);

        while($recordedte = mysqli_fetch_array($resultedte)){

            $ennprid = $recordedte['nprid'];
            $ensgid = $recordedte['sgid'];
            $enscwsdate = $recordedte['scwsdate'];
            $enmsubject = $recordedte['msubject'];
            $enowgrade = $recordedte['owgrade'];
            $entid = $recordedte['tid'];

        }

        $updatetr="UPDATE nonprinciple SET sgid='$ensgid',scwsdate='$enscwsdate',msubject='$enmsubject',owgrade='$enowgrade',tid='$entid' where nprid='$ennprid' ";
        $conn-> query($updatetr);

        $deletetnpr="delete from nonprincipletemp where nprid= $ennprid";
                                ...
                                <table id="example23" class="display nowrap" cellspacing="0" width="100%">
                                    ...
                                    <?php
                                        while($recordjub = mysqli_fetch_array($resultjub)){   
                                            $msubjub = $recordjub['msubject'];
                                            $sgidp = $recordjub['sgid'];
                                            $owgr = $recordjub['owgrade'];
                                            $tniv = $recordjub['tid'];
                                                ...
                                                $sqlzcl="SELECT * FROM classtbllet where cleid = $owgr";
                                                $resultzcl=$conn->query($sqlzcl);
                                                while($recordzcl = mysqli_fetch_array($resultzcl)){
                                                    $clidol = $recordzcl['clid'];
                                                    $letterol = $recordzcl['letter'];
                                                    $deletterol = base64_decode($letterol);
                                                    $sqlcnm="SELECT * FROM classt where clid = $clidol";
                                                    $resultcnm=$conn->query($sqlcnm);
                                                    while($recordcnm = mysqli_fetch_array($resultcnm)){
                                                        $classol = $recordcnm['class'];
                                                        $declassol = base64_decode($classol);
                                                    }
                                                }

                                                $sqlgrf="SELECT * FROM serandgrade where sgid = $sgidp";
                                                $resultgrf=$conn->query($sqlgrf);
                                                while($recordgrf = mysqli_fetch_array($resultgrf)){
                                                    $gradeol = $recordgrf['grade'];
                                                    $degradeol = base64_decode($gradeol);
                                                }
                                    ...
                                    ?>
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            <td><a href="<?php echo "teacherEdit.php?nprid="?><?php echo $recordjub['nprid']; ?>" ><button class="btn btn-primary">Approve</button></a></td>
                                            <td><?php echo $denicol; ?></td>
                                            <td><?php echo $degradeol; ?></td> 
                                            <td><?php echo $recordjub['scwsdate']; ?></td>
                                            <td><?php echo $deblbv." (".$deblstjo." - ".$debljtoc." )"; ?></td>
                                            <td><?php echo $declassol." - ".$deletterol; ?></td>
                                        </tr>
                                        <?php
                                            }
                                        ?>
                                ...
                                <table id="example26" class="display nowrap" cellspacing="0" width="100%">
                                    ...
                                    <?php
                                        while($recorddef = mysqli_fetch_array($resultdef)){   
                                            $msubdef = $recorddef['msubject'];
                                            $sgidpdef = $recorddef['sgid'];
                                            $owgrdef = $recorddef['owgrade'];
                                            ...
                                            $sqlcdth="SELECT * FROM classtbllet where cleid = $owgrdef";
                                            $resultcdth=$conn->query($sqlcdth);
                                            while($recordcdth = mysqli_fetch_array($resultcdth)){
                                                $cldg = $recordcdth['clid'];
                                                $letterdf = $recordcdth['letter'];
                                                $deletterdf = base64_decode($letterdf);
                                                $sqlcnj="SELECT * FROM classt where clid = $cldg";
                                                $resultcnj=$conn->query($sqlcnj);
                                                while($recordcnj = mysqli_fetch_array($resultcnj)){
                                                    $classcnj = $recordcnj['class'];
                                                    $declasscnj = base64_decode($classcnj);
                                                }
                                            }

                                            $sqlsgo="SELECT * FROM serandgrade where sgid = $sgidpdef";
                                            $resultsgo=$conn->query($sqlsgo);
                                            while($recordsgo = mysqli_fetch_array($resultsgo)){
                                                $gradesgo = $recordsgo['grade'];
                                                $degradesgo = base64_decode($gradesgo);
                                            }
                                    ...
                                    ?>
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            <td><a href="<?php echo "teacherEdit.php?delnprid="?><?php echo $recorddef['nprid']; ?>" ><button class="btn btn-primary">Approve</button></a></td>
                                            <td><?php echo $denictcg; ?></td>
                                            <td><?php echo $degradesgo; ?></td> 
                                            <td><?php echo $recorddef['scwsdate']; ?></td>
                                            <td><?php echo $decprbv." (".$deblsush." - ".$deblscon." )"; ?></td>
                                            <td><?php echo $declasscnj." - ".$deletterdf; ?></td>
                                        </tr>
                                        <?php
                                            }
                                        ?>
