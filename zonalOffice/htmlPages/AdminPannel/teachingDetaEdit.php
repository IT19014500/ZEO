...
<?php
  ...
  $sqljub="SELECT * FROM nptteachtemp";
...
<?php
  ...
  $sqldef="SELECT * FROM nptteachtmdel";
  $resultdef=$conn->query($sqldef);

?> 

<?php
if(isset($_GET['delnptid'])){

    $denpo = $_GET['delnptid'];

    $deletetnact="delete from nptteach where nptid= $denpo";
    $conn->query($deletetnact);

    $deletetnvb="delete from nptteachtmdel where nptid= $denpo";
...
<?php
    if(isset($_GET['gnptid'])){
        
        $bytedi = $_GET['gnptid'];

        $sqledte="SELECT * FROM nptteachtemp WHERE nptid = $bytedi";
        $resultedte=$conn->query($sqledte);

        while($recordedte = mysqli_fetch_array($resultedte)){

            // $ennptid = $recordedte['nptid'];
            $ensuID = $recordedte['suID'];
            $enowgrade = $recordedte['owgrade'];
            $enperiod = $recordedte['period'];
            $entid = $recordedte['tid'];
        }

        $updatetr="UPDATE nptteach SET suID='$ensuID',owgrade='$enowgrade',period='$enperiod',tid='$entid' where nptid= $bytedi ";
        $conn-> query($updatetr);

        $deletetnpr="delete from nptteachtemp where nptid= $bytedi";
                                ...
                                <table id="example23" class="display nowrap" cellspacing="0" width="100%">
                                    ...
                                    ?>
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            <td><a href="<?php echo "teachingDetaEdit.php?gnptid="?><?php echo $recordjub['nptid']; ?>" ><button class="btn btn-primary">Approve</button></a></td>
                                            <td><?php echo $denicol; ?></td>
                                            <td><?php echo $deblbv." (".$deblstjo." - ".$debljtoc." )"; ?></td>
                                            <td><?php echo $declassol." - ".$deletterol; ?></td>
                                            <td><?php echo $recordjub['period']; ?></td>
                                        </tr>
                                        <?php
                                            }
                                        ?>
                                ...
                                <table id="example26" class="display nowrap" cellspacing="0" width="100%">
                                    ...
                                    <?php
                                        while($recorddef = mysqli_fetch_array($resultdef)){
                                            ...
                                            $sqlcdv="SELECT * FROM classtbllet where cleid = $owgrdef";
                                            $resultcdv=$conn->query($sqlcdv);
                                            while($recordcdv = mysqli_fetch_array($resultcdv)){
                                                $clidcdv = $recordcdv['clid'];
                                                $lettercdv = $recordcdv['letter'];
                                                $delettercdv = base64_decode($lettercdv);
                                                $sqlchl="SELECT * FROM classt where clid = $clidcdv";
                                                $resultchl=$conn->query($sqlchl);
                                                while($recordchl = mysqli_fetch_array($resultchl)){
                                                    $classchl = $recordchl['class'];
                                                    $declasschl = base64_decode($classchl);
                                                }
                                            }
                                    ...
                                    ?>
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            <td><a href="<?php echo "teachingDetaEdit.php?delnptid="?><?php echo $recorddef['nptid']; ?>" ><button class="btn btn-primary">Approve</button></a></td>
                                            <td><?php echo $denictcg; ?></td>
                                            <td><?php echo $decprbv." (".$deblsush." - ".$deblscon." )"; ?></td>
                                            <td><?php echo $declasschl." - ".$delettercdv; ?></td>
                                            <td><?php echo $recorddef['period']; ?></td>
                                        </tr>
                                        <?php
                                            }
                                        ?>
