...
<?php
  ...
  $sqljub="SELECT * FROM extractvitmp";
...
<?php
  ...
  $sqldef="SELECT * FROM extractvitmdel";
  $resultdef=$conn->query($sqldef);

?>

<?php
if(isset($_GET['defeide'])){

    $denpo = $_GET['defeide'];

    $deletetnact="delete from extractvi where eid= $denpo";
    $conn->query($deletetnact);

    $deletetnvb="delete from extractvitmdel where eid= $denpo";
...
<?php
    if(isset($_GET['eide'])){
        ...
        $sqledte="SELECT * FROM extractvitmp WHERE eid = $bytedi";
        $resultedte=$conn->query($sqledte);

        while($recordedte = mysqli_fetch_array($resultedte)){

            $TextAct=$recordedte['extAct'];
            $Ttid=$recordedte['tid'];

        }

        $updatetr="UPDATE extractvi SET extAct='$TextAct',tid='$Ttid' where eid=$bytedi ";
        $conn-> query($updatetr);

        $deletetnpr="delete from extractvitmp where eid= $bytedi";
                                ...
                                <table id="example23" class="display nowrap" cellspacing="0" width="100%">
                                    ...
                                    ?>
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            <td><a href="<?php echo "extraAct.php?eide="?><?php echo $recordjub['eid']; ?>" ><button class="btn btn-primary">Approve</button></a></td>
                                ...
                                <table id="example26" class="display nowrap" cellspacing="0" width="100%">
                                    ...
                                    <?php
                                        while($recorddef = mysqli_fetch_array($resultdef)){   
                                            ...
                                            $juenbadef = $recorddef['extAct'];
                                            $deenbadef = base64_decode($juenbadef);
                                    ...
                                    ?>
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            <td><a href="<?php echo "extraAct.php?defeide="?><?php echo $recorddef['eid']; ?>" ><button class="btn btn-primary">Approve</button></a></td>
                                            <td><?php echo $denictcg; ?></td>
                                            <td><?php echo $deenbadef; ?></td> 
                                        </tr>
                                        <?php
                                            }
                                        ?>
