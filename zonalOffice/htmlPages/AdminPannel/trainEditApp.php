...
<?php
  $sqljub="SELECT * FROM tstbltemp";
  $resultjub=$conn->query($sqljub);
?>

<?php
  $sqldef="SELECT * FROM tstbltmdel";
  $resultdef=$conn->query($sqldef);
?> 

<?php
if(isset($_GET['deftic'])){

    $denpo = $_GET['deftic'];

    $deletetnact="delete from tstbl where tid= $denpo";
    $conn->query($deletetnact);

    $deletetnvb="delete from tstbltmdel where tid= $denpo";
...
<?php
    if(isset($_GET['tidg'])){
        ...
        $sqledte="SELECT * FROM tstbltemp WHERE tid = $bytedi";
        $resultedte=$conn->query($sqledte);

        while($recordedte = mysqli_fetch_array($resultedte)){

            $Ttid = $recordedte['tid'];
            $entsts = $recordedte['tsts'];
            

        }

        $updatetr="UPDATE tstbl SET tsts='$entsts' where tid=$Ttid ";
        $conn-> query($updatetr);

        $deletetnpr="delete from tstbltemp where tid= $Ttid";
                                ...
                                <table id="example23" class="display nowrap" cellspacing="0" width="100%">
                                    ...
                                    <?php
                                        while($recordjub = mysqli_fetch_array($resultjub)){   
                                            $tstsjub = $recordjub['tsts'];
                                            $tniv = $recordjub['tid'];

                                            $detstsjub = base64_decode($tstsjub);
                                    ...
                                    ?>
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            <td><a href="<?php echo "trainEditApp.php?tidg="?><?php echo $recordjub['tid']; ?>" ><button class="btn btn-primary">Approve</button></a></td>
                                            <td><?php echo $denicol; ?></td>
                                            <td><?php echo $detstsjub; ?></td>
                                        </tr>
                                        <?php
                                            }
                                        ?>
                                ...
                                <table id="example26" class="display nowrap" cellspacing="0" width="100%">
                                    ...
                                    <?php
                                        while($recorddef = mysqli_fetch_array($resultdef)){   
                                            $tstsdef = $recorddef['tsts'];
                                            $tnivdef = $recorddef['tid'];

                                            $detstsdef = base64_decode($tstsdef);
                                    ...
                                    ?>
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            <td><a href="<?php echo "trainEditApp.php?deftic="?><?php echo $recorddef['tid']; ?>" ><button class="btn btn-primary">Approve</button></a></td>
                                            <td><?php echo $denicvde; ?></td>
                                            <td><?php echo $detstsdef; ?></td>
                                        </tr>
                                        <?php
                                            }
                                        ?>
