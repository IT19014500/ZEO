    ...
    try{
        ...
        while($recordbyta = mysqli_fetch_array($resultbyta)){
            ...
            if($astri->format("%a") > $ghti){
                ...
                $deletebyd="delete from exmdutytmdel where id= $bytaid";
...
<?php 
        ...
        while($recordbytane = mysqli_fetch_array($resultbytane)){
            ...
            if($astrine->format("%a") > $ghti){
                $deletebydne="delete from exmdutytmp where id= $bytaidne";
...
<?php
        ...
        echo "Basic data reading failed!";
    }
?>
        ...
        <div style="background-color:#acb0bd;" class="page-wrapper">
            <div class="container-fluid">
                ...
                <div class="col-lg-2 col-sm-4 col-xs-12">
                    <a href="exDutyExp.php"><button class="btn btn-block btn-primary">Add</button></a>
                </div><br><br>
                <h1 style="color:#000000;text-align:center;"><strong><em>Exam Duty Details</em></strong></h1><br>
                <div class="row">
                <div class="col-sm-12">
                                        ...
                                        <?php
                                            try{
                                                ...
                                                while($recordbmn = mysqli_fetch_array($resultbmn)){
                                                    ...
                                                    $sql3="SELECT * FROM exmduty WHERE tid = $fhg";
                                        ...
                                        ?>
                                                <?php
                                                    while($record3 = mysqli_fetch_array($result3)){
                                                        $tbit= $record3['tid'];
                                                        $enid = $record3['id'];
                                                        $enedyear = $record3['edyear'];
                                                        $enexname = $record3['exname'];
                                                        $enprofession = $record3['profession'];
                                                        $enexcenter = $record3['excenter'];
                                                        ...
                                                        $sqljao="SELECT * FROM exmduty where tid = $tbit && tid NOT IN (SELECT tid FROM ofisign);";
                                                        $resultjao=$conn->query($sqljao);
                                                        while($recordjao = mysqli_fetch_array($resultjao)){
                                                            $balka = "No";
                                                        }
                                                        $sqlbno="SELECT * FROM exmduty where id = $enid && id IN (SELECT id FROM exmdutytmp);";
                                                        $resultbno=$conn->query($sqlbno);
                                                        while($recordbno = mysqli_fetch_array($resultbno)){
                                                            $balka = "ReqEdit";
                                                        }

                                                        $sqljpo="SELECT * FROM exmduty where id = $enid && id IN (SELECT id FROM exmdutytmdel);";
                                        ...
                                        ?>
                                                <tr style="color:#000000;background-color:#ffffff;">
                                                    <?php
                                                        if($balka=="No"){
                                                    ?>
                                                    <td><a href="../../Action/examDuty/exDUpdate.php?id=<?php echo $record3['id']; ?>"><button class = "btn btn-primary">EDIT</button></a>
                                                    <a href="../../Action/examDuty/exdDelete.php?id=<?php echo $record3['id']; ?>"><button class = "btn btn-danger">DELETE</button></a></td>
                                                    <?php
                                                    ...
                                                    ?>        
                                                    <?php
                                                        $sqlbhl="SELECT * FROM teacher where tid = $tbit";
                                                    ...
                                                    <td><?php echo $deenedyear; ?></td>
                                                    <td><?php echo $deenexname; ?></td>
                                                    <td><?php echo $deenprofession; ?></td>
                                                    <td><?php echo $deenexcenter; ?></td> 
                                                </tr>
                                                <?php
                                                    }
                                                }
                                            }catch(Exception $e) {
                                                echo "Data Reading Process Fail!";
                                            }
                                        ?>
                                    </tbody>
                                </table>
