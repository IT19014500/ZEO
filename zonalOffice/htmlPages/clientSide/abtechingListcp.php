...
<?php 
  ...
  while($recordbyta = mysqli_fetch_array($resultbyta)){
    ...
    if($astri->format("%a") > $ghti){
      $deletebyd="delete from nptteachtmdel where nptid= $bytaid";
...
<?php 
  ...
  if($astrine->format("%a") > $ghti){
      $deletebydne="delete from nptteachtemp where nptid= $bytaidne";
        ...
        <div style="background-color:#acb0bd;" class="page-wrapper">
            <div class="container-fluid">
                ...
                <div class="col-lg-2 col-sm-4 col-xs-12">
                    <a href="nprteachcs.php"><button class="btn btn-block btn-primary">Add</button></a>
                </div>
                <h1 style="color:#000000;text-align:center;"><strong><em>Manage Teaching Details</em></strong></h1><br>
                <div class="row">
                <div class="col-sm-12">
                        <div style="background-color:#729ca6;" class="white-box">
                            <h3 class="box-title m-b-0">Teaching Details</h3>
                            <div class="table-responsive">
                                <table id="example23" class="display nowrap" cellspacing="0" width="100%">
                                    <caption style="color:#000000;">Teaching details</caption>
                                    <thead>
                                        <tr>
                                            <!-- <th>NPTID</th> -->
                                            <th>PHOTO</th>
                                    ...
                                    <?php
                                        ...
                                        while($recordbmn = mysqli_fetch_array($resultbmn)){
                                            ...
                                            $sql3="SELECT * FROM nptteach WHERE tid = $fhg";
                                    ...
                                    <?php
                                        ...
                                        $sqljao="SELECT * FROM nptteach where tid = $tbit && tid NOT IN (SELECT tid FROM ofisign);";
                                        $resultjao=$conn->query($sqljao);
                                        while($recordjao = mysqli_fetch_array($resultjao)){
                                            $balka = "No";
                                        }
                                        $sqlbno="SELECT * FROM nptteach where nptid = $onptidd && nptid IN (SELECT nptid FROM nptteachtemp);";
                                        $resultbno=$conn->query($sqlbno);
                                        while($recordbno = mysqli_fetch_array($resultbno)){
                                            $balka = "ReqEdit";
                                        }

                                        $sqljpo="SELECT * FROM nptteach where nptid = $onptidd && nptid IN (SELECT nptid FROM nptteachtmdel);";
                                        ...
                                        <tr style="color:#000000;background-color:#ffffff;">
                                            ...
                                            <?php
                                                ...
                                                $resultowg=$conn->query($sqlowg);
                                                    ...
                                                    $clio = $recordowg['clid'];
                                                    $sqlcml="SELECT * FROM classt where clid = $clio";
                                                    $resultcml=$conn->query($sqlcml);
                                                    while($recordcml = mysqli_fetch_array($resultcml)){
                                                        $ocnm = $recordcml['class'];
                                                        $deocnm = base64_decode($ocnm);
                                                    }

                                                    $letterme = $recordowg['letter'];
                                                    $deletterme = base64_decode($letterme);
                                                    
                                            ?>
                                            <td><?php echo $deocnm." - ".$deletterme; ?></td>
                                            <?php
                                                }
                                            ?>
                                            <td><?php echo $record3['period']; ?></td>
                                            <?php
                                                $wqo = $record3['tid'];
                                            ...
                                            <?php
                                                if($balka=="No"){
                                            ?>
                                            <td><a href="../../Action/npteaching/nptUpdate.php?id=<?php echo $record3['nptid']; ?>"><button class = "btn btn-primary">EDIT</button></a>
                                            <br><br> <a href="../../Action/npteaching/nptDelete.php?id=<?php echo $record3['nptid']; ?>"><button class = "btn btn-danger">DELETE</button></a></td>
                                            <?php
