	...
	class QRcode {
		...
		*/
		...
		protected function encode_rs_char($rs, $data, $parity) {
			...
			$A0       =& $NN;
			$parityupdt = array_fill(0, $NROOTS, 0);
			for ($i=0; $i < ($NN - $NROOTS - $PAD); $i++) {
				$feedback = $INDEX_OF[$data[$i] ^ $parityupdt[0]];
				if ($feedback != $A0) {
					...
					for ($j=1; $j < $NROOTS; ++$j) {
					$parityupdt[$j] ^= $ALPHA_TO[$this->modnn($rs, $feedback + $GENPOLY[($NROOTS - $j)])];
					}
				}
				// Shift
				array_shift($parityupdt);
				if ($feedback != $A0) {
					array_push($parityupdt, $ALPHA_TO[$this->modnn($rs, $feedback + $GENPOLY[0])]);
				} else {
					array_push($parityupdt, 0);
				}
			}
			return $parityupdt;
		}
